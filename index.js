// Rolbox Command Server - PARANOID EDITION
const express = require('express');
const bodyParser = require('body-parser');
const cors = require('cors');
const mysql = require('mysql2/promise');
const argon2 = require('argon2'); // <<< SECURITY UPGRADE >>> Stronger password hashing
...
const session = require('express-session'); // <<< SECURITY UPGRADE >>> For session management and instant logout
const KnexSessionStore = require('connect-session-knex')(session);
const knex = require('knex'); // <<< SECURITY UPGRADE >>> For session store
const csrf = require('csurf'); // <<< SECURITY UPGRADE >>> CSRF protection
const { nanoid } = require('nanoid'); // <<< SECURITY UPGRADE >>> Secure ID generation
...
const STRIKE_TIMEFRAME_MS = 15 * 60 * 1000; // 15 minutes
...
app.use((req, res, next) => {
    if (['/connect', '/poll'].includes(req.path)) {
        return next();
    }
...
app.post('/register', authLimiter,
    ...
    body('password').isLength({ min: 12 }).withMessage('Password must be at least 12 characters long.').matches(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{12,}$/).withMessage('Password must contain uppercase, lowercase, number, and special character.'),
    async (req, res, next) => {
        try {
...
app.post('/login', authLimiter, async (req, res, next) => {
    try {
        ...
        if (rows.length === 0 || !(await argon2.verify(rows[0].password_hash, password))) {
            addStrike(req.ip);
...
app.post('/broadcast', verifyToken, verifyRole('admin'), actionLimiter, async (req, res, next) => { 
    try {
...
setInterval(() => {
    ...
    for (const [ip, expiry] of ipBanList.entries()) {
        if (now > expiry) { ipBanList.delete(ip); console.log(`[SECURITY] Unbanned IP: ${ip}`); }
    }
    for (const [ip, record] of strikeTracker.entries()) {
        if (now - record.firstStrikeTimestamp > STRIKE_TIMEFRAME_MS) { strikeTracker.delete(ip); }
    }
}, 5 * 60 * 1000);
