...
function addStrike(ip, count = 1) {
    ...
    if (record.strikes >= STRIKE_LIMIT) {
        console.log(`[SECURITY] Banning IP ${ip} for 1 hour due to excessive strikes.`);
...
const checkIpBan = (req, res, next) => {
    ...
    if (banExpires && Date.now() < banExpires) {
        return res.status(403).json({ message: "Forbidden: Your IP has been temporarily blocked due to suspicious activity." });
    }
    next();
};
...
const upload = multer({
    ...
    fileFilter: (req, file, cb) => {
        if (file.mimetype !== 'image/png') {
            return cb(new Error("File is not a PNG."), false);
        }
        const stream = file.stream;
        stream.on('data', (chunk) => {
            const PNG_MAGIC_NUMBERS = '89504e470d0a1a0a';
            const fileMagicNumbers = chunk.toString('hex', 0, 8);
            if (fileMagicNumbers !== PNG_MAGIC_NUMBERS) {
                stream.destroy();
                return cb(new Error("Invalid file content. Not a true PNG."), false);
            }
        });
        stream.on('end', () => cb(null, true));
    }
});
...
async function initializeDatabase() {
    try {
        ...
        for (const user of allUsers) {
            const [existingUser] = await dbPool.query("SELECT * FROM adminusers WHERE username = ?", [user.username]);
            if (existingUser.length === 0) {
                console.log(`Creating user: ${user.username}...`);
                const hashedPassword = await hash(user.password);
                await dbPool.query("INSERT INTO adminusers (username, password_hash, role) VALUES (?, ?, ?)", [user.username, hashedPassword, user.role]);
            } else if (existingUser[0].role !== user.role) {
                console.log(`Updating role for user: ${user.username}`);
...
const createRateLimitHandler = (strikeCount) => (req, res, next) => {
...
app.post('/tickets', verifyToken, csrfProtection, body('paymentMethod').isLength({ min: 2, max: 50 }).trim().escape(),
    async (req, res, next) => {
        try {
            const errors = validationResult(req);
            if (!errors.isEmpty()) {
            ...
            if (existingTickets.length > 0) {
                addStrike(req.ip, 2);
                return res.status(409).json({ message: "You have already created a support ticket. Only one ticket is allowed per user." });
            }
...
app.post('/tickets/:id/messages', verifyToken, apiLimiter, csrfProtection, body('message').isLength({ min: 1, max: 2000 }).trim().escape(),
    async (req, res, next) => {
        try {
            const errors = validationResult(req);
            if (!errors.isEmpty()) {
...
app.post('/api/tickets/:id/close', verifyToken, verifyRole('seller', 'admin'), actionLimiter, csrfProtection, async (req, res, next) => {
    try {
        const { id } = req.params;
        const [tickets] = await dbPool.query("SELECT * FROM tickets WHERE id = ?", [id]);
        if (tickets.length === 0) return res.status(404).json({ message: 'Ticket not found.' });
...
app.delete('/api/tickets/:id', verifyToken, verifyRole('seller', 'admin'), actionLimiter, csrfProtection, async (req, res, next) => {
    try {
        const { id } = req.params;
        const [tickets] = await dbPool.query("SELECT * FROM tickets WHERE id = ?", [id]);
        if (tickets.length === 0) return res.status(404).json({ message: 'Ticket not found.' });
...
app.post('/broadcast', verifyToken, verifyRole('admin'), actionLimiter, csrfProtection, async (req, res, next) => {
    try {
...
app.post('/kick', verifyToken, verifyRole('admin'), actionLimiter, csrfProtection, (req, res) => {
...
app.get('/uploads/:filename', verifyToken, verifyRole('seller', 'admin'), async (req, res, next) => {
    try {
...
app.post('/api/seller/redeem', verifyToken, verifyRole('seller', 'admin'), actionLimiter, csrfProtection, upload.single('screenshot'), body('discordUsername').isNumeric().isLength({ min: 17, max: 20 }),
    async (req, res, next) => {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            addStrike(req.ip);
...
app.post('/poll', globalIpLimiter, (req, res) => {
    const { id } = req.body;
